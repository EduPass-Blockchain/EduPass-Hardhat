...
contract CertificateContract is ERC721URIStorage {
    ...
    modifier onlyRole(RoleManagerContract.RoleType role) {
        require(
            roleManager.getRole(msg.sender) == role,
            ErrorCodes.ERROR_UNAUTHORIZED
        );
        _;
    }
    ...
    function createCertificate(
        ...
        public
        onlyRole(RoleManagerContract.RoleType.ORGANIZATION)
        returns (uint256)
    {
    ...
    function listAllCertificatesOfUser(
        address userAddr
        ...
        for (uint256 i = 0; i < listToken.length; i++) {
            if (
                (userAddr == msg.sender && !listToken[i].isPublic) ||
                roleManager.getRole(msg.sender) ==
                RoleManagerContract.RoleType.ADMIN ||
                listToken[i].isPublic
            ) {
    ...
    function setCertificatePublic(
        string memory userTokenUri
    ...
    function setCertificatePrivate(
        string memory userTokenUri
    ) public isOwner(msg.sender, userTokenUri) {
        UserToken[] storage listToken = listOfUsersToken[msg.sender];
        for (uint256 i = 0; i < listToken.length; i++) {
